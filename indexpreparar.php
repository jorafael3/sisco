...
echo "<H2>Listado facturas por preparar:<br></H2>";
...
//  and factura <>'' and estado ='Facturado' and preparada='' and DATE_FORMAT(fechafact,'%H:%i:%s') <>'00:00:00' order by secuencia desc ";
if ($_SESSION["canal_id"] > 2) {
	...
	where a.despacho ='' 
	and a.despachofinal ='' 
	and a.anulada <>'1' and a.canal = ".$_SESSION["canal_id"]."
	and a.factura <>'' and a.estado <>'' and DATE_FORMAT(a.preparada,'%H:%i:%s') ='00:00:00' 
	order by a.secuencia desc";
} else {
	$sql = "SELECT a.*, d.bodega as bodegaret FROM covidsales as a
	left join covidpickup as d on d.orden= a.secuencia
	where a.despacho ='' and a.despachofinal ='' and a.anulada <>'1' 
	and a.factura <>'' and a.estado <>'' and DATE_FORMAT(a.preparada,'%H:%i:%s') ='00:00:00' 
	order by a.secuencia desc";
}
...
while ($row = mysqli_fetch_array($result)) {
	...
	echo "<td>" . $row['bodegaret'] . "<br>" . $dataentrega . "</td>";
	...
	if ($row['factura'] <> '' and $nivel == "30" and $row['despacho'] == '' and ($row['estado'] <> 'Verif. pago')) {
		echo "<td><a href=preparar1.php?sec=$sec&numfac=$factura>" . $row['factura'] . "</td>";
	} else {
		echo "<td>" . $row['factura'] . "</td>";
	}

	if (($row['factura'] <> '') and ($nivel == "30") and ($row['despachofinal'] == '') and ($row['bultos'] <> '')) {
		echo "<td><a href=preparar1.php?sec=$sec&numfac=$factura>" . $row['fechafact'] . "</td>";
	} else {
